                ...
                .inorder=F, .errorhandling="remove", .options.mpi=mpiopts) %dopar% {
                  ...
                  for (a in factor.algorithm) {
                    if(results.list[[as.character(a)]]$solved==0) {
                      ...
                      infos.a <- lapply(infos, function(i, ind) i[ind,,], ind = results.list[[as.character(a)]]$ind.opt)
                  ...
                  saveRDS(res.r, file=paste0("results_opt_grid1/results_simulation_opt_grid1_d",d,".rds"))
                  res.r
                }

saveRDS(res, file="results_simulation_opt_grid1.rds")
