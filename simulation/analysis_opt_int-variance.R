...
# 
# mean.frame(dvs = c("rec", "RMSE", "MAB", "rel", "fisherz.r"), ivs = c("blocksize"), results = res, rn = 3)
...
print(xtable::xtable(means.paper, digits=2,
                     caption="Mean trait recovery by condition in simulation study 2 on test construction for the equal and weighted targets (population test)",
                     label="tb:means_rec"), include.colnames = F, include.rownames=F, 
...
saveRDS(res, file = "simulation/results_opt_int-variance_cleaned.rds")
...
# lm.algo.opt.main <- calc.lms.main(dvs=c("A","D","T.opt","Frob"), ivs=c("algorithm","intercepts","target","blocksize"), results=res)
# var.expl(lm.algo.opt.main)
# 
# lm.algo.opt <- calc.lms(dvs=c("A","D","T.opt","Frob"), ivs=c("algorithm","intercepts","target","blocksize"), results=res)
# var.expl.algo.opt <- var.expl(lm.algo.opt)
...
rownames(var.paper) <- c("Algorithm vs. Random", "Optimality vs. Means", "$R^2$ vs. Mean Variances", 
...
print(xtable::xtable(var.paper, digits=0,
                     caption="Variance in trait recovery explained in \\% by algorithm, target and intercepts in simulation study 2 on test construction for the equal and weighted targets (population test)",
                     label="tb:var_rec"), include.colnames = F, include.rownames=T, hline.after=c(0, nrow(var.paper)-1),
      sanitize.rownames.function=function(x){x}, sanitize.colnames.function = function(x){x},
      sanitize.text.function = function(x){x},
      NA.string = "", table.placement = "htp", add.to.row = header,
      caption.placement = "top", latex.environments = NULL,
...
ggsave("simulation/plot_opt_int-variance_equal-ordered-3.pdf",
