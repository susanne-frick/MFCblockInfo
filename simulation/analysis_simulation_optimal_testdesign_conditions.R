...
res <- readRDS("simulation/results_simulation_optimal_testdesign_conditions_part1.rds")
...
#4 are missing, probably they were removed because there was an error
...
#save
# saveRDS(id.res, file="missing_rows_opt_conditions.rds")

#number of NAs (optimizer did not find a solution)
table(rowSums(is.na(res))>0)
res[rowSums(is.na(res))>0,]
#3 tests for block size 2

#add missing rows which were run by now
res.miss <- readRDS("simulation/results_simulation_opt_conditions_missingrows.rds")
