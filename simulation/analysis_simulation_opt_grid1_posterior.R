...
res_variance <- readRDS("simulation/results_simulation_opt_grid1_posterior_r2-avg.rds")
res_random <- readRDS("simulation/results_simulation_opt_grid1_posterior_int-random.rds")
head(res)

# remove rows with r2-avg and r2-avg-sd
res_variance <- res_variance[!(res_variance$algorithm %in% c("r2-avg", "r2-avg-sd")),]
table(res_variance$algorithm)
table(res_random$algorithm)

# merge both
res <- rbind(res_variance, res_random)
# encode in new/old variable intercepts
res$intercepts <- c(rep("ordered", nrow(res_variance)), rep("random", nrow(res_random)))
rm(res_variance, res_random)
