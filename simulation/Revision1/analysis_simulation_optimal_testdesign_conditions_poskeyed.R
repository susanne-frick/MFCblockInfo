...
# res <- readRDS("Projekte/MFC_blocks/MFCblockInfo_simulations/results_simulation_optimal_testdesign_conditions_small.rds")
res <- readRDS("simulation/results_simulation_optimal_testdesign_conditions_poskeyed_part1.rds")
# res <- readRDS("simulation/results_simulation_optimal_testdesign_conditions_testlido.rds")
head(res)
...
#2 are missing, have not finished yet
#all for block size 2 and unconstrained
3*2*2*200
table(res$rep)
which(table(res$rep)<240)
#identifier for which ones are missing
# id.res <- paste(res$blocksize, res$target, res$constraints, res$rep, sep="_")
#save
# saveRDS(id.res, file="missing_rows_opt_conditions_poskeyed.rds")

#number of NAs (optimizer did not find a solution)
table(rowSums(is.na(res))>0)
res[rowSums(is.na(res))>0,]
#5 tests for block size 2, all in the unconstrained case

#add missing rows which were run by now
res.miss <- readRDS("simulation/results_simulation_optimal_testdesign_conditions_poskeyed_part2.rds")
...
print(xtable::xtable(means.paper, digits=2,
                     caption="Mean trait recovery by condition in the simulation on test construction with all positively keyed items",
      ...
      file="../../Projekte/MFC_blocks/paper/Revision1_Psychometrika/SOM/textable_means_construction_poskeyed.tex")
...
header$command <- c("\\hline \n Factor & $r(\\theta, \\hat{\\theta})$ & MAB & RMSE\\\\",
                    "\\hline \n \\multicolumn{4}{l}{\\small \\textit{Note.} MAB = Mean Absolute Bias, RMSE = Root Mean Squared} \\\\
                    \\multicolumn{4}{l}{\\small Error, MIP T = Mixed Integer Programming with T-optimality. } \\\\
                    \\multicolumn{4}{l}{\\small $r(\\theta, \\hat{\\theta})$ was Fisher \\textit{Z} transformed.}\n")

print(xtable::xtable(var.paper, digits=0,
                     caption="Variance in trait recovery explained in \\% by algorithm, target and constraints in 
                     the simulation on test construction with all positively keyed items",
                     label="tb:var_sim_con"), include.colnames = F, include.rownames=T, hline.after=c(0, nrow(var.paper)-1),
      sanitize.rownames.function=function(x){x}, sanitize.colnames.function = function(x){x},
      sanitize.text.function = function(x){x},
      NA.string = "", table.placement = "htp", add.to.row = header,
      caption.placement = "top", latex.environments = NULL,
      file="../../Projekte/MFC_blocks/paper/Revision1_Psychometrika/SOM/textable_var_construction_poskeyed.tex")
...
res.requal.con3 <- res[res$target=="equal" & res$constraints=="constrained" & res$blocksize==3,]
res.requal.con2 <- res[res$target=="equal" & res$constraints=="constrained" & res$blocksize==2,]

plot.MAB3 <- ggplot(data=res.requal.con3, aes(y=MAB, x=algorithm, fill=algorithm)) +
...
plot.RMSE3 <- ggplot(data=res.requal.con3, aes(y=RMSE, x=algorithm, fill=algorithm)) +
  geom_violin(show.legend=FALSE) +
  ...
  theme(axis.text=element_text(size=11),
        axis.title=element_text(size=11))
plot.rec3 <- ggplot(data=res.requal.con3, aes(y=rec, x=algorithm, fill=algorithm)) +
  geom_violin(show.legend=FALSE) +
...
ggsave("../../Projekte/MFC_blocks/paper/Revision1_Psychometrika/SOM/plot_optimal_testdesign_poskeyed_B3.pdf",
       grid.arrange(plot.MAB3, plot.RMSE3, plot.rec3, nrow=1, ncol=3),
       width=10, height=4, units="in")

####------------ block size 2
plot.MAB2 <- ggplot(data=res.requal.con2, aes(y=MAB, x=algorithm, fill=algorithm)) +
  geom_violin(show.legend=FALSE) +
  labs(y="MAB", x="Algorithm") +
  scale_x_discrete(labels = c('opt' = "MIP T",
                              'r2'   = expression(plain(Block)~R^2),
  ...
  theme(axis.text=element_text(size=11),
        axis.title=element_text(size=11))
plot.RMSE2 <- ggplot(data=res.requal.con2, aes(y=RMSE, x=algorithm, fill=algorithm)) +
  geom_violin(show.legend=FALSE) +
  labs(y="RMSE", x="Algorithm") +
  scale_x_discrete(labels = c('opt' = "MIP T",
                              'r2'   = expression(plain(Block)~R^2),
  ...
  theme(axis.text=element_text(size=11),
        axis.title=element_text(size=11))
plot.rec2 <- ggplot(data=res.requal.con2, aes(y=rec, x=algorithm, fill=algorithm)) +
  geom_violin(show.legend=FALSE) +
  labs(y=expression(r(theta,hat(theta))), x="Algorithm") +
  scale_x_discrete(labels = c('opt' = "MIP T",
                              'r2'   = expression(plain(Block)~R^2),
...
ggsave("../../Projekte/MFC_blocks/paper/Revision1_Psychometrika/SOM/plot_optimal_testdesign_poskeyed_B2.pdf",
       grid.arrange(plot.MAB2, plot.RMSE2, plot.rec2, nrow=1, ncol=3),
       width=10, height=4, units="in")
