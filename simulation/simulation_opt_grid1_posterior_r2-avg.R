...
factor.length <- "long"
factor.algorithm <- c("greedy-a","greedy-d","opt-t","mean-a","r2","r2-avg","r2-avg-sd","loads","random")
...
res <- foreach (d=1:nrow(design.sim), .combine=rbind, .verbose=T, .packages=c("mvtnorm","numDeriv","devtools","lpSolveAPI","MFCblockInfo"),
                .inorder=F, .errorhandling="remove", .options.mpi=mpiopts) %dopar% {
                  ...
                  #R^2 mean with rep random subsets of R^2
                  r2.avg <- lapply(1:K, function(k, ...) calc.info.block.r2(wo.blocks = k, ...),
                                   info.all = infos, 
                                   avg = TRUE, rep = 200,
                                   K.final = K.final, constraint.list = constraint.list)
                  
                  means.r2.avg <- do.call(c, lapply(r2.avg, function(r2) mean(rowMeans(r2$mean.r2)*a.all)))
                  sd.r2.avg <- do.call(c, lapply(r2.avg, function(r2) mean(rowMeans(r2$mean.sd.r2)*a.all)))
                  
                  means.r2.avg <- matrix(means.r2.avg, 1, K)
                  sd.r2.avg <- matrix(sd.r2.avg, 1, K)
                  
                  ind.r2.avg <- select.optimal(info.sum=means.r2.avg, traits.grid=matrix(0,1,1), K=K, K.final=K.final,
                                               constraint.list=constraint.list)$ind.opt
                  ind.r2.avg.sd <- select.optimal(info.sum=sd.r2.avg, traits.grid=matrix(0,1,1), K=K, K.final=K.final,
                                               constraint.list=constraint.list)$ind.opt
                  ...
                  results.list <- list("greedy-a"=ind.a.opt, "greedy-d"=ind.d.opt, "opt-t"=ind.t.opt,
                                       "mean-a"=ind.a.mean, "r2"=ind.r2, "r2-avg"=ind.r2.avg, "r2-avg-sd"=ind.r2.avg.sd, 
                                       "loads"=ind.loads, "random"=ind.rand)
                  testinfo <- colSums(infos[[1]], dims = 1)
                  ...
                  for (a in factor.algorithm) {
                    if(all(results.list[[as.character(a)]])) {
                      ...
                      A <- calc.a.optimality(infos.a)
                      D <- calc.d.optimality(infos.a)
                      testinfo.a <- colSums(infos.a[[1]], dims = 1)
                  ...
                  saveRDS(res.r, file=paste0("results_opt_grid1_posterior_r2-avg/results_opt_grid1_posterior_d",d,".rds"))
                  res.r
                }

saveRDS(res, file="results_simulation_opt_grid1_posterior_r2-avg.rds")
