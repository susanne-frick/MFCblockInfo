...
res <- readRDS("simulation/results_simulation_opt_poskeyed_dependent_part1.rds")
head(res)

anyNA(res)

#all conditions included?
table(res$blocksize, useNA="always") #quite a few are missing for block size 2 and unconstrained
table(res$keying)
table(res$length)
...
nrow(res)/(5*4)
#there should be 2400 / 1600
#all for block size 2
3*2*2*200

#number of NAs (optimizer did not find a solution)
table(rowSums(is.na(res))>0) #none
...
ggsave("../../Projekte/MFC_blocks/paper/Revision1_Psychometrika/SOM/plot_optimal_testdesign_poskeyed_B3_dependent.pdf",
...
ggsave("../../Projekte/MFC_blocks/paper/Revision1_Psychometrika/SOM/plot_optimal_testdesign_poskeyed_B2_dependent.pdf",
