...
res <- foreach (d=1:nrow(design.sim), .combine=rbind, .verbose=T, .packages=c("mvtnorm","numDeriv","devtools","lpSolveAPI","MFCblockInfo"),
                .inorder=F, .errorhandling="remove", .options.mpi=mpiopts) %dopar% {
                    ...
                    ind.a.opt <- select.greedy(infos, calc.a.optimality, traits.grid, K, K.start, K.final, maximize=FALSE)
                    ind.d.opt <- select.greedy(infos, calc.d.optimality, traits.grid, K, K.start, K.final, maximize=TRUE)
                    ...
                    results.opt <- select.optimal(info.trace, traits.grid, info.trace.pool.start, K-K.start, K.final-K.start)
                    ...
                    saveRDS(res.r, file=paste0("results_greedy/results_greedy_d",d,".rds"))
                    res.r
                }

saveRDS(res, file="results_simulation_greedy.rds")
